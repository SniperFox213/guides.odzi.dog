...
<Popover arrow={ false } placement="bottom-end" bind:open={ isOpen } overlayColor="rgba(0,0,0,0)">
  ...
  <div in:fade style="min-width: 20vw;" slot="content" class="py-4">
    <div in:slide class="w-full h-full rounded-md bg-white shadow-lg">
      <!-- User Info -->
      <div class="w-full flex flex-col justify-center items-center px-3 pt-3">
        <div class="flex">
          <!-- Avatar -->
        ...
        <button on:click={(e) => {
          settings.changeSetting("minimalisticView", false);
        }} class="mt-2 w-full flex justify-center items-center text-gray-800 py-2 px-6 rounded-md border-1 border-solid border-gray-300">
          <!-- Icon -->
          <img style="height: 1.2rem; width: 1.2rem;" src="./icons/test-tube.png" alt="Beta Mode Icon">

          <!-- Text -->
          <p class="text-sm ml-2">Перейти в <span class="mx-1 text-xs rounded-lg bg-red-300 text-white px-2">Beta</span> версию</p>
        </button>
      </div>
